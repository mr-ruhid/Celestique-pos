...
<div class="h-[calc(100vh-120px)] flex flex-col md:flex-row gap-4"
     x-data="{
        ...
        isLoading: false,
        isProcessing: false,
        showCheckoutModal: false, // Modal üçün dəyişən
        promoCode: '', // Promokod üçün
        ...
        async processPayment(method) {
            ...
            try {
                ...
                if (result.success) {
                    // DÜZƏLİŞ: Alert əvəzinə avtomatik çap pəncərəsini açırıq
                    const printUrl = '/sales/' + result.order_id + '/print-official';

                    // Popup pəncərədə çapı açmaq (Daha professional görünür)
    ...
    <div class="w-full md:w-1/3 flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 h-full">
        ...
        <div class="flex-1 overflow-y-auto p-2 space-y-2">
            <template x-for="(item, index) in cart" :key="item.id">
                <div class="bg-gray-50 p-3 rounded-lg border border-gray-200 flex justify-between items-center group hover:bg-white hover:shadow-sm transition">
                    <div class="flex-1">
                        ...
                        <div class="text-xs text-gray-500 mt-1 flex gap-2">
                            ...
                            <template x-if="item.discount_amount > 0">
                                <span class="text-red-500 text-[10px]">(Endirimli)</span>
                            </template>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <div class="text-right w-16">
                            <p class="text-sm font-bold text-gray-800" x-text="formatPrice(item.final_price * item.qty)"></p>
                        </div>
                        <button @click="removeFromCart(index)" class="text-red-400 hover:text-red-600 w-6 text-center">
